...
- <커피숍에 비유>
  ...
  - 액션생성: 주문서 작성 (아메리카노 하나랑 라떼 하나 주세요)
    - 페이로드: 주문서 작성 (아메리카노는 **샷추가**, 라떼 우유는 **두유**로 바꿔주세요)
  - 디스패치 함수: 주문하기
  - 리듀서 함수: 주문 받은걸 만들고 가공해서 손님에게 내어 줌
...
const CounterReducer = () => {
  ...
  // useReducer의 큰 장점: 컴포넌트 업데이트 로직을 컴포넌트 바깥으로 빼낼 수 있음
...
- 그래서 e.target 값 자체를 액션 값으로 사용해 보자
...
- 이 Hook을 사용하면 만들어놨던 함수를 재사용할 수 있음
- 컴포넌트가 리렌더링될 때마다 새로 만들어진 함수를 사용하게 되는 것은 렌더링이 자주 발생하거나 랜더링해야 할 컴포넌트의 개수가 많아지면 최적화하는 것이 좋음
...
- 여기서 로컬 변수는 렌더링과 상관 없이 바뀔 수 있는 **값**을 의미
- 실습예제: 더블클릭 방지 기능 구현, 사용자가 버튼을 빠르게 여러 번 클릭하는 경우 예상치 못한 여러 번의 액션을 방지
